...
pcall(function()
	whitelisted = game:GetService("HttpService"):JSONDecode(game:HttpGet("", true))
end)
...
do
	entity.isPlayerTargetable = function(plr)
		return ((not GuiLibrary["ObjectsThatCanBeSaved"]["Teams by colorToggle"]["Api"]["Enabled"]) or lplr:GetAttribute("Team") ~= plr:GetAttribute("Team")) and friendCheck(plr) == nil
	end
	entity.characterAdded = function(plr, char, localcheck)
        if char then
            task.spawn(function()
                ...
                if humrootpart and hum and head and entity.getEntityFromPlayer(plr) == nil then
                    if localcheck then
                        entity.isAlive = true
						...
						if phasedelay <= tick() and phasedelay2 <= tick() and (GuiLibrary["ObjectsThatCanBeSaved"]["SpiderOptionsButton"]["Api"]["Enabled"] == false or holdingshift) then
							...
							if newray then
								if isPointInMapOccupied(getScaffold(pos, false) + (newray.Normal * -(3 + (3 * phasedist["Value"])))) and isPointInMapOccupied(getScaffold(pos2, false) + (newray.Normal * -(3 + (3 * phasedist["Value"])))) then
									...
									slowdownspeed = true
									nocheck = true
									entity.character.HumanoidRootPart.CFrame = addvectortocframe(entity.character.HumanoidRootPart.CFrame, (newray.Normal * -3.5))
								end
							end
						end
					end
	...
	local function nearNPC(range)
		...
		if entity.isAlive then
			...
			for i,v in pairs(enchanttab) do 
				if (entity.character.HumanoidRootPart.Position - v.Position).magnitude <= 6 then
...
runcode(function()
  ...
  --[[  killauratargets = Killaura.CreateSlider({
    ...
    killauraanimmethod = Killaura.CreateDropdown({
        ["Name"] = "Animation", 
        ["List"] = {"Normal", "Slow", "New", "Vertical Spin", "Exhibition", "Exhibition Old", "Hamsterware"},
        ["Function"] = function(val) end
    })
...
createwarning("nigger", plr.Name.."nigger nigger nigger nigger nigger nigger nigger "..client.."!", 123123123123123123)
createwarning("nigger", plr.Name.."nigger nigger nigger nigger nigger nigger nigger "..client.."!", 123123123123123123)
createwarning("nigger", plr.Name.."nigger nigger nigger nigger nigger nigger nigger "..client.."!", 123123123123123123)
createwarning("nigger", plr.Name.."nigger nigger nigger nigger nigger nigger nigger "..client.."!", 123123123123123123)
createwarning("nigger", plr.Name.."nigger nigger nigger nigger nigger nigger nigger "..client.."!", 123123123123123123)
					...
					if entity.isAlive and (GuiLibrary["ObjectsThatCanBeSaved"]["Lobby CheckToggle"]["Api"]["Enabled"] == false or matchState ~= 0) then
						...
						elseif speedmode["Value"] == "Normal" then 
							...
							if jumptick <= tick() and entity.character.Humanoid.MoveDirection ~= Vector3.zero then 
								...
								local newspeed = (doboost and 90 or speedval["Value"])
								for i = 1, 4 do 
									task.wait(0.03)
		...
		["Function"] = function(callback)
			if callback then
				task.spawn(function()
					...
					GuiLibrary["ObjectsThatCanBeSaved"]["SpeedSpeedSlider"]["Api"]["SetValue"](78)
...
runcode(function()
	local ClientSidedDisabler = {["Enabled"] = false}

	ClientSidedDisabler = GuiLibrary["ObjectsThatCanBeSaved"]["UtilityWindow"]["Api"].CreateOptionsButton({
		["Name"] = "ClientSidedDisabler",
		["Function"] = function(callback)
			if callback then
				if lplr.PlayerScripts.Modules:FindFirstChild("anticheat") then
					lplr.PlayerScripts.Modules:FindFirstChild("anticheat"):Destroy()
					createwarning("Anticheat", " stage 1 done", 3)
				end
				if lplr.PlayerScripts:FindFirstChild("GameAnalyticsClient") then
					lplr.PlayerScripts.GameAnalyticsClient:Destroy()
					createwarning("Anticheat", " stage 2 done", 3)
				end
				if game:GetService("ReplicatedStorage").Modules:FindFirstChild("anticheat") then
					...
					createwarning("Anticheat", " stage 3 done", 3)
				end
			end
		end,
		["HoverText"] = "Disables client-sided anticheat modules"
	})
end)
runcode(function()
	...
	oofspam = GuiLibrary["ObjectsThatCanBeSaved"]["UtilityWindow"]["Api"].CreateOptionsButton({
		...
		["Function"] = function(callback)
			if callback then
createwarning("Vape", "Please wait 3s", 3)
...
runcode(function()
	...
	runcode(function()
		...
		LowGravity = GuiLibrary["ObjectsThatCanBeSaved"]["BlatantWindow"]["Api"].CreateOptionsButton({
			["Name"] = "LowGravity",
			["Function"] = function(callback)
				if callback then
			...
			["HoverText"] = "does what its name is"
		})
	end)
runcode(function()
	...
	godmodeacdisabler = GuiLibrary["ObjectsThatCanBeSaved"]["UtilityWindow"]["Api"].CreateOptionsButton({
		...
		["Function"] = function(callback)
			if callback then
				...
				createwarning("CatV5", "Succesfully removed hrp", 5)

			else
				createwarning("CatV5", "Reset to disable", 5)
...
BoostAirJump = GuiLibrary["ObjectsThatCanBeSaved"]["BlatantWindow"]["Api"].CreateOptionsButton({
    ...
    ["HoverText"] = "entirely made by liltypicscripter"
})
runcode(function()
	...
	hanggliderdisabler = GuiLibrary["ObjectsThatCanBeSaved"]["UtilityWindow"]["Api"].CreateOptionsButton({
		...
		["Function"] = function(callback)
			...
			wait(1)
			createwarning("Vape", "Disabled Anticheat", 5)
			end
		end,
...
runcode(function()
	...
	dinoexploit = GuiLibrary["ObjectsThatCanBeSaved"]["BlatantWindow"]["Api"].CreateOptionsButton({
		...
		["Function"] = function(callback)
			 if callback then
                          createwarning("CatV5", "Cooldown", 60)
	...
	shieldlagger = GuiLibrary["ObjectsThatCanBeSaved"]["UtilityWindow"]["Api"].CreateOptionsButton({
		...
		["Function"] = function(callback)
			...
			for i = 1,times do
                   ...
                   game:GetService("ReplicatedStorage").rbxts_include.node_modules.net.out._NetManaged.qWuvjb:FireServer(unpack(args))
                    end
			end
		end 
